...
do
  for j in {0..100}
  do
    ERROR=$(./build/default/main -p $i 2>&1 1>/dev/null)
    if [ $? -eq 0 ]
    then
      ...
      then
        echo "./build/default/main -p $i: GOOD OUTPUT"
      else
        echo "./build/default/main -p $i: BAD OUTPUT"
      fi
    else
      echo "./build/default/main -p $i: $ERROR"
    fi
  done
done
